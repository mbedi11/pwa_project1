...
function setOnlineBadge() {
  ...
  el.style.background = online ? 'rgba(34,197,94,0.14)' : 'rgba(245,158,11,0.14)';
  el.style.borderColor = online ? 'rgba(34,197,94,0.35)' : 'rgba(245,158,11,0.35)';
}
...
function wireInstallPrompt() {
  window.addEventListener('beforeinstallprompt', (e) => {
    ...
    $('#installHint').textContent = 'Aplikacija je installable. U Chrome-u klikni Install (ikonica u address baru) ili ovaj gumb kad ga dodamo.';
  });
...
async function enablePush() {
  ...
  try {
    ...
    console.error(err);
    statusEl.textContent = 'Neuspjelo uključivanje push-a (provjeri HTTPS na cloudu).';
...
async function startCameraOrFallback() {
  ...
  try {
    ...
    note.textContent = 'Ne mogu pristupiti kameri (dozvola/uređaj). Koristi fallback upload.';
    btnPick.style.display = 'inline-block';
...
async function registerSync() {
  if (swReg && swReg.sync && navigator.onLine === false) {
    // Registering while offline is a common pattern
    try {
...
async function renderQueue() {
  ...
  list.innerHTML = '';
  if (items.length === 0) {
    list.innerHTML = '<div class="hint">Nema queued fotki. Snimi jednu, pa prebaci Network → Offline i testiraj.</div>';
    return;
  }
...
window.addEventListener('load', async () => {
  ...
  if (!supportsCamera) {
    ...
    $('#btnPickFile').addEventListener('click', () => $('#fileFallback').click());
  }
});
